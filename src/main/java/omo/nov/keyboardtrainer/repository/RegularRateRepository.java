...
public interface RegularRateRepository extends JpaRepository<RegularRate, UUID> {
    ...
    @Query(value = "SELECT new omo.nov.keyboardtrainer.payload.RegularDTO(rg.id , rg.limitSecondRegulate , rg.trueLetterCount , rg.falseLetterCount , rg.startAt , rg.endAt) FROM RegularRate rg order by rg.commonTrue desc")
