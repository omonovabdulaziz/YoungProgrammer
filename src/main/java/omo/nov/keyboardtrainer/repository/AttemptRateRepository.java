...
public interface AttemptRateRepository extends JpaRepository<AttemptRate, UUID> {
    ...
    @Query(value = "SELECT new omo.nov.keyboardtrainer.payload.AttemptRateDTO(ar.user , ar.trueLetterCount , ar.falseLetterCount , ar.commonTrue , ar.startAt , ar.endAt) FROM AttemptRate ar where ar.contest.id=:contest_id and ar.user.status=true and ar.falseLetterCount < 20 order by ar.commonTrue desc")
